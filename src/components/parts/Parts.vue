<template>
  <div>
    ...
    <div class="content table-responsive">
      ...
      <table
        class="table table-striped table-bordered b-table-fixed"
...
export default {
  ...
  methods: {
    deletePart(id, index) {
      UserService.deletePart("http://127.0.0.1:1025/endpoint/part/" + id).then(
        () => {
    ...
    searchPart() {
      UserService.searchPart(
        "http://127.0.0.1:1025/endpoint/part/search?type=" +
          this.type +
          "&query=" +
          this.query +
          "&size=" +
