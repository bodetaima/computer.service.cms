        ...
        <div class="container">
            ...
            <b-form>
                ...
                <div style="display: grid;">
                    <div :key="computer.id" class="card"
                         v-for="(computer, index) in computers">
                        <ComputerCreate/>
                        <b-button @click="deleteComputerCard(index)" style="margin: 5px" v-if="index > 0"
                                  variant="danger">
                            Xóa máy tính
                        </b-button>
                    </div>
                    <b-button @click="addNewComputerCard" style="margin: 5px" v-if="computers.length < 5"
                              variant="info">
                        Thêm máy tính
                    </b-button>
                </div>
...
<script>
    ...
    export default {
        name: "CreateOrder",
        components: {Navbar, ComputerCreate},
        data() {
            return {
                computers: [
                    {id: 1}
        ...
        methods: {
            addNewComputerCard() {
                this.computers.push({id: this.computers.length + 1})
            },
